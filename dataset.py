...
class Dataset:
    ...
    def include_date(self, dataDate):
        """ 
            ...
            with respect to params.years and params.months
        """
        dataDate_ = str(dataDate)
        year = int(dataDate_[:4])
        month = int(dataDate_[4:6])
        
        if year not in self.params.years:
            return False
            
        if month not in self.params.months:
            return False
...
class DatasetSingle(Dataset):
    """
        ...
        @param start_lon: the longitude of the point
        @param start_lat: the latitude of the point
        @param years: only load data from those years
        @param months: only load data from those months
    """
    def __init__(self, params):
        params.end_lon = params.start_lon
        params.end_lat = params.start_lat
    ...
    def load_frames(self):
        ...
        while 1:
            ...
            nearest = codes_grib_find_nearest(gid, self.params.start_lat, self.params.start_lon)[0]
    ...
    def create_sets(self):
        ...
        train_size = int(len(self.frames) * (1 - self.params.test_fraction))
        ...
        self.trainX, self.trainY = self.create_dataset(train, self.params.window_size)
        self.testX, self.testY = self.create_dataset(test, self.params.window_size)
...
class DatasetMultiple(Dataset):
    ...
    def load_frames(self):
        ...
        while 1:
            ...
            if not self.include_date(codes_get(gid, "dataDate")):
                continue
            ...
            #print self.params.start_lat, self.params.start_lon
