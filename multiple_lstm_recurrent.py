...
def example():
    ...
    model = Sequential()  
    model.add(LSTM(hidden_neurons, input_shape=(in_out_neurons, window_size), return_sequences=False))  
    ...
    trainPredictPlot[window_size:len(trainPredict)+window_size, :] = trainPredict[:,:1]
    
    # shift test predictions for plotting
    testPredictPlot = np.empty_like(data.frames)
    testPredictPlot[:, :] = np.nan
    testPredictPlot[len(trainPredict)+(window_size*2)+1:len(data.frames)-1, :] = testPredict[:,:1]
